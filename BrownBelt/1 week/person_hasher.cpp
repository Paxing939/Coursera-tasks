...
struct Person {
  ...
  bool operator==(const Person &other) const {
    return name == other.name
           && height == other.height
           && abs(weight - other.weight) < std::numeric_limits<double>::epsilon()
...
struct AddressHasher {
  size_t operator()(const Address &address) const {
    size_t x = 47;
    return string_hasher(address.city) * x * x + string_hasher(address.street) * x + int_hasher(address.building);
  }
...
struct PersonHasher {
  size_t operator()(const Person &person) const {
    size_t x = 47;
    return string_hasher(person.name) * x * x * x + address_hasher(person.address) * x * x
           + int_hasher(person.height) * x + double_hasher(person.weight);
  }
