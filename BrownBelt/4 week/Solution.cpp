    ...
    for (auto y = 0; y < size.height; ++y)
      for (auto x = 0; x < size.width; ++x)
        if (IsPointInShape({x, y})) {
          if (Point img_point = {point.x + x, point.y + y}; IsPointIn(img_point, image)) {
            if (texture && IsPointIn({x, y}, *texture)) {
              image[img_point.y][img_point.x] = texture->GetImage()[y][x];
            } else {
              image[img_point.y][img_point.x] = pixel;
            }
            std::cout << "(" << img_point.y << ", " << img_point.x << ")" << std::endl;
          }
        }
  }
